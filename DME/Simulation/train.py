...
from DME.Simulation.src.model import DMEGP
from DME.Simulation.src.means import MLP, LSTM, GRU
from DME.Simulation.src.data_loader import fetch_dataloaders
from DME.Simulation.src.utils import get_mean_params, get_kernel_params, init_weights
...
def train_single_epoch(model, dataloader, mean_optim, gp_optim, loss_fn_fixed,
    ...
    loss = 0.
    count = 0
    for (train_X, train_y, _, train_Z, n) in dataloader:
        train_X = train_X[0]
...
def evaluate(model, dataloader, loss_fn, n_adapt, inner_lr, val=True,
             interpolation=False):
    F_rmse, rmse, loss = 0., 0., 0.
    count = 0
    if interpolation:
        for (X, y, F, Z, n_test) in dataloader:
            ...
            for t in reversed(range(1, n_test+1)):
                ...
                F_rmse += (F_pred.item() - test_F.item())**2
                count += 1
    else:
        for (X, y, F, Z, _) in dataloader:
            # sequential predictions using GP
            X = X[0]
            y = y.view(-1)
            F = F.view(-1)
...
def train_and_evaluate(model, dataloaders, mean_optim, gp_optim, loss_fn_fixed, loss_fn,
    ...
    with tqdm(total=n_epochs) as td:
        for i in range(n_epochs):
            ...
            _, error_val, loss_val = evaluate(model, dl_val, loss_fn, n_adapt,
                                              inner_lr)
...
def train_and_test_model(model_config, train_config, dataframes,
                         random_effects_column_names,
                         group_column_name, y_column_name,
                         n_samples_chosen_per_group,
                         model_type='MLP',
    ...
    # test
    F_rmse_test1, rmse_test1, loss_test1 = evaluate(model, dataloader_test1, loss_fn, n_adapt,
                                                    inner_lr, val=False)

    F_rmse_test2, rmse_test2, loss_test2 = evaluate(model, dataloader_test2, loss_fn, n_adapt,
    ...
    return F_rmse_test1, F_rmse_test2, rmse_test1, rmse_test2
