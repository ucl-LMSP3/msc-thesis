...
from src.means import MLP, LSTM, GRU
from src.data_loader import fetch_dataloaders
from src.utils import get_mean_params, get_kernel_params, init_weights
    ...
    if interpolation:
        for (X, y, F, Z, n_test) in dataloader:
            ...
            for t in reversed(range(1, n_test+1)):
                ...
                pred, pred_var, loss_step = model(train_y-F_pred, train_Z, test_Z,
                                                  loss_fn, inner_lr,
                                                  n_adapt)
                loss += loss_step
