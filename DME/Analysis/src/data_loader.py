...
class DeepMEDataset(Dataset):
    def __init__(self, dataframe, random_effects_column_names,
        ...
        self.input_dim = dataframe.shape[1] - self.random_effects_dim - 2
        self.n_samples_chosen_per_group = n_samples_chosen_per_group
        self.X, self.Y, self.Z = self.load_data()

    def __len__(self):
        return len(self.X)

    def __getitem__(self, idx):
        return self.X[idx], self.Y[idx], self.Z[idx], self.n_samples_chosen_per_group[idx]
    ...
    def load_data(self):
        ...
        for group in grouped_data:
            columns_to_be_dropped = self.random_effects_column_names + [self.group_column_name, self.y_column_name]
            X = group.drop(columns=columns_to_be_dropped).to_numpy(dtype=np.float32)
